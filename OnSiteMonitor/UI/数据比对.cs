...
{
    public partial class 数据比对 : Form
    {
        ...
        {
            if (File.Exists(GlobalEnviroment.ResultOutputDir + "\\日志.txt"))
                File.Delete(GlobalEnviroment.ResultOutputDir + "\\日志.txt");
        ...
        private void DataComparing(Models.Task _t)
        {
            ...
            Business.FundComparing comparor = new Business.FundComparing();
            comparor.IsDataChecking = IsDataCheck;

            if (!comparor.Start(_t))
            {
                log.Info("------------------------------------------------------");
